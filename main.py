...
RENDER_URL = os.environ.get("RENDER_EXTERNAL_URL", "")  # Render خودش اینو ست می‌کنه
...
def admin_buttons(user_id, is_blocked):
    if is_blocked:
        btn = {"text": "✅ آنبلاک", "callback_data": f"unblock:{user_id}"}
    else:
        btn = {"text": "⛔ بلاک", "callback_data": f"block:{user_id}"}
    return {"inline_keyboard": [[btn]]}


# ============ هندلرها ============
def handle_start(user, chat_id):
    if user.id == ADMIN_ID:
        bot.send_message(
            chat_id=chat_id,
            text=(
    ...
    if db.is_blocked(user.id):
        bot.send_message(chat_id, "⛔ شما توسط مدیریت مسدود شده‌اید.")
        return
...
def handle_user_message(message, user):
    ...
    try:
        ...
        bot.send_message(message.chat.id, "✅ ارسال شد. منتظر جواب باش.")
...
def handle_admin_message(message, user):
    ...
    if text == "/blocked":
        ...
        if not rows:
            bot.send_message(ADMIN_ID, "لیست مسدودها خالیه ✅")
            return
        t = "⛔ کاربران مسدود:\n\n"
    ...
    if text == "/stats":
        ...
        bot.send_message(ADMIN_ID, f"📊 کل: {total}\n⛔ مسدود: {blocked}\n✅ فعال: {total - blocked}")
        return
    ...
    if text.startswith("/block "):
        try:
            ...
            return
        if uid == ADMIN_ID:
            bot.send_message(ADMIN_ID, "خودت رو نمی‌تونی بلاک کنی 😅")
            return
        db.block_user(uid)
        bot.send_message(ADMIN_ID, f"⛔ کاربر {uid} بلاک شد.")
        try:
            bot.send_message(uid, "⛔ شما توسط مدیریت مسدود شدید.")
        except Exception:
            pass
        return
    ...
    if text.startswith("/unblock "):
        try:
            ...
            bot.send_message(ADMIN_ID, "استفاده: /unblock <user_id>")
            return
        db.unblock_user(uid)
        bot.send_message(ADMIN_ID, f"✅ کاربر {uid} آنبلاک شد.")
        try:
            bot.send_message(uid, "✅ شما آنبلاک شدید.")
        except Exception:
            pass
        return
    ...
    if text.startswith("/find "):
        ...
        if not rows:
            bot.send_message(ADMIN_ID, "چیزی پیدا نشد.")
            return
        t = f"🔍 نتایج «{esc(query)}»:\n\n"
    ...
    if text == "/broadcast":
        bot.send_message(ADMIN_ID, "📣 برای ارسال همگانی، کافیه پیام مورد نظر رو بفرستی (بدون دستور).")
        return

    if text == "/cancel":
        bot.send_message(ADMIN_ID, "❌ لغو شد.")
        return
    ...
    if message.reply_to_message:
        ...
        if not target_id:
            ...
            return
        try:
            bot.send_message(target_id, "📬 پاسخ مدیریت:\n➖➖➖➖➖➖")
            bot.copy_message(
                chat_id=target_id,
            ...
            bot.send_message(ADMIN_ID, f"⚠️ خطا: {e}")
        return

    bot.send_message(ADMIN_ID, "برای جواب، روی پیام کاربر ریپلای بزن ↩️")
    ...
    if action == "block":
        ...
        try:
            bot.edit_message_reply_markup(
                ...
                reply_markup={"inline_keyboard": [[
                    {"text": "✅ آنبلاک", "callback_data": f"unblock:{uid}"}
                ]]}
            )
        except Exception:
            pass
        bot.answer_callback_query(cb.id, "بلاک شد")
        try:
        ...
        try:
            bot.edit_message_reply_markup(
                ...
                message_id=cb.message.message_id,
                reply_markup={"inline_keyboard": [[
                    {"text": "⛔ بلاک", "callback_data": f"block:{uid}"}
                ]]}
            )
        except Exception:
            pass
        bot.answer_callback_query(cb.id, "آنبلاک شد")
        try:
            bot.send_message(uid, "✅ آنبلاک شدید.")
        except Exception:
            pass
...
# ============ ست کردن Webhook موقع بالا اومدن ============
def setup_webhook():
    if not RENDER_URL:
        print("⚠️ RENDER_EXTERNAL_URL ست نشده، Webhook نمی‌تونه تنظیم بشه.")
        return
    webhook_url = f"{RENDER_URL}/webhook"
    try:
        bot.delete_webhook()
        result = bot.set_webhook(url=webhook_url, drop_pending_updates=True)
        print(f"✅ Webhook ست شد: {webhook_url} | نتیجه: {result}")
    except Exception as e:
